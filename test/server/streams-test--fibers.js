/*** Generated by streamline fibers 0.1.0 - DO NOT EDIT ***/var fstreamline__ = require("streamline/lib/fibers/runtime"); (fstreamline__.create(function(_) {var doTest_ = fstreamline__.create(doTest), dot_ = fstreamline__.create(dot); //streamline.options = { "lines" : "preserve" }
"use strict";
...
new streams.HttpServer( fstreamline__.create(function (req, res, _) {
	res.writeHead(200, {'Content-Type': 'application/octet-stream'});
	res.emitter.on("drain", function() {
		process.stderr.write("*");
	})
	for (var i = 0; i < bufCount; i++) {
		fstreamline__.invoke(res, "write", [_, makeBuffer(i)], 0);
		fstreamline__.invoke(process, "nextTick", [_], 0);
	}
	res.end();
}, 2)).listen(null, 1337, "127.0.0.1");
...
function doTest(_, name, options, fn) {
	...
	var resp =  fstreamline__.invoke(streams.httpRequest(options).end(), "response", [_], 0);
	addBufferHooks(resp.emitter);
	fstreamline__.invoke(null, fn, [_, resp], 0);
	var last = fstreamline__.invoke(resp, "read", [_], 0);
...
function testPass(name, options) {
	...
	function testRead(name, detail, size) {
		asyncTest(name + " / " + detail, fstreamline__.create(function(_) {
			fstreamline__.invoke(null, doTest_, [_, name, options, fstreamline__.create(function(_, resp) {
				for (var i = 0, total = 0; total < totalSize; i++) {
					var len = size && typeof size === "function" ? size() : size;
					var buf = fstreamline__.invoke(resp, "read", [_, len], 0);
...
asyncTest("stop  tests", 0, fstreamline__.create(function(_) {
	doStop = true;
	start();
}, 0));
