/*** Generated by streamline 0.1.36-stack - DO NOT EDIT ***/ var __global = typeof global !== 'undefined' ? global : window;var __fastCb=false; function __cb(_, fn){ var ctx = __global.__context; var frame = new Error(); frame.__prev = __global.__frame; return function ___(err, result){ __global.__frame = frame; __global.__context = ctx; if (err) { err.__prev = err.__prev || frame; return _(err); } frame.__valid = true; if (__fastCb) return fn(null, result); try { return fn(null, result); } catch (ex) { ex.__prev = ex.__prev || frame; return __propagate(_, ex); } } } function __future(fn, args, i){ var done, err, result; var cb = function(e, r){ done = true; err = e, result = r; }; args = Array.prototype.slice.call(args); args[i] = function ___(e, r){ cb(e, r); }; fn.apply(this, args); return function ___(_){ if (done) _.call(this, err, result); else cb = _.bind(this); } .bind(this); } function __nt(_, fn){ var i = 0; var cb = __cb(_, fn); var safeCb = function ___(){ try { cb(); } catch (ex) { __propagate(cb, ex); } }; if (typeof process != "undefined" && typeof process.nextTick == "function") return function ___(){ if (++i % 20 == 0) process.nextTick(safeCb); else cb(); }; else return function ___(){ if (++i % 20 == 0) setTimeout(safeCb); else cb(); }; } function __propagate(_, err){ try { _(err); } catch (ex) { __trap(ex); } } function __trap(err){ if (err) { if (__global.__context && __global.__context.errorHandler) __global.__context.errorHandler(err); else console.error("UNCAUGHT EXCEPTION: " + err.message + "\n" + err.stack); } } function __tryCatch(_, fn){ try { fn(); } catch (e) { try { _(e); } catch (ex) { __trap(ex); } } } var module = QUnit.module;




var flows = require("streamline/lib/util/flows");

function nextTick(cb) {
 setTimeout(function() {
 cb();
 }, 0);};


function failAsync(_, code) { if (!_) { return __future(failAsync, arguments, 0); };
 return _(new Error(code));};


function failSync(_, code) { if (!_) { return __future(failSync, arguments, 0); };
 (function fail(dummy) {
 throw new Error(code);
 })(0); _();};


var fail;

function A(_, code) { if (!_) { return __future(A, arguments, 0); }; var i; return (function __$A(__then) {
 if ((code == 1)) {
 return fail(__cb(_, __then), code); } else { __then(); } ; })(function __$A() { return (function __$A(__then) {
 if ((code == 2)) {
 return fail(__cb(_, __then), code); } else { __then(); } ; })(function __$A() {
 return nextTick(__cb(_, function __$A() { return (function __$A(__then) {
 if ((code == 3)) {
 return fail(__cb(_, __then), code); } else { __then(); } ; })(function __$A() {
 i = 0; var __6 = false; return (function ___(__break) { var __loop = __nt(_, function __$A() { if (__6) { i++; } else { __6 = true; } ; var __5 = (i < 6); if (__5) { return (function __$A(__then) {
 if ((code == i)) {
 return fail(__cb(_, __then), code); } else { __then(); } ; })(function __$A() {
 return nextTick(__cb(_, __loop)); }); } else { __break(); } ; }); __loop(); })(function __$A() { return (function __$A(__then) {

 if ((code == 6)) {
 return fail(__cb(_, __then), code); } else { __then(); } ; })(function __$A() {
 return nextTick(__cb(_, function __$A() {
 return B(__cb(_, function __$A() {
 return nextTick(__cb(_, function __$A() {
 return _(null, "END"); })); }), code); })); }); }); }); })); }); });};


function B(_, code) { if (!_) { return __future(B, arguments, 0); }; return (function __$B(__then) {
 if ((code == 7)) {
 return fail(__cb(_, __then), code); } else { __then(); } ; })(function __$B() {
 return C(__cb(_, function __$B() {
 return nextTick(__cb(_, function __$B() {
 return C(__cb(_, function __$B() {
 return D(_, code); }), code); })); }), code); });};


function C(_, code) { if (!_) { return __future(C, arguments, 0); }; return (function __$C(__then) {
 if ((code == 8)) {
 return fail(__cb(_, __then), code); } else { __then(); } ; })(_);};


function D(_, code) { if (!_) { return __future(D, arguments, 0); }; return (function __$D(__then) {
 if ((code == 9)) {
 return fail(__cb(_, __then), code); } else { __then(); } ; })(_);};


function T(_, code, failFn) { if (!_) { return __future(T, arguments, 0); }; var s, ff;
 fail = failFn;
 s = "{"; return (function ___(__then) { (function ___(_) { __tryCatch(_, function __$T() {

 return A(_, code); }); })(function ___(ex, __result) { __tryCatch(_, function __$T() { if (ex) {


 s = flows.stackTrace(ex);


 s = s.split("\n").map(function(l) {

 var m = /^\s+at (\w+)\s\(.*:(\d+)\:.*\)/.exec(l);
 if (m) {
 return ((m[1] + ":") + m[2]) };

 m = /^([^(]+)\([^)]+\)@.*\:(\d+)$/.exec(l);


 if (m) {
 ff = true;
 return ((m[1] + ":") + m[2]); } ;

 return l;
 }).join("/");


 return _(null, (ff ? ((("Error: " + code) + "/") + s) : s.substring(0, s.indexOf("/T:")))); } else { _(null, __result); } ; }); }); })(function ___() { __tryCatch(_, _); });};



module("flows");

asyncTest("stacks", 20, function __1(_) { if (!_) { return __future(__1, arguments, 0); };
 return T(__cb(_, function ___(__0, __1) { strictEqual(__1, "Error: 1/failAsync:15/A:28");
 return T(__cb(_, function ___(__0, __2) { strictEqual(__2, "Error: 1/fail:20/failSync:21/A:28");
 return T(__cb(_, function ___(__0, __3) { strictEqual(__3, "Error: 2/failAsync:15/A:30");
 return T(__cb(_, function ___(__0, __4) { strictEqual(__4, "Error: 2/fail:20/failSync:21/A:30");
 return T(__cb(_, function ___(__0, __5) { strictEqual(__5, "Error: 3/failAsync:15/A:33");
 return T(__cb(_, function ___(__0, __6) { strictEqual(__6, "Error: 3/fail:20/failSync:21/A:33");
 return T(__cb(_, function ___(__0, __7) { strictEqual(__7, "Error: 4/failAsync:15/A:36");
 return T(__cb(_, function ___(__0, __8) { strictEqual(__8, "Error: 4/fail:20/failSync:21/A:36");
 return T(__cb(_, function ___(__0, __9) { strictEqual(__9, "Error: 5/failAsync:15/A:36");
 return T(__cb(_, function ___(__0, __10) { strictEqual(__10, "Error: 5/fail:20/failSync:21/A:36");
 return T(__cb(_, function ___(__0, __11) { strictEqual(__11, "Error: 6/failAsync:15/A:40");
 return T(__cb(_, function ___(__0, __12) { strictEqual(__12, "Error: 6/fail:20/failSync:21/A:40");
 return T(__cb(_, function ___(__0, __13) { strictEqual(__13, "Error: 7/failAsync:15/B:49/A:42");
 return T(__cb(_, function ___(__0, __14) { strictEqual(__14, "Error: 7/fail:20/failSync:21/B:49/A:42");
 return T(__cb(_, function ___(__0, __15) { strictEqual(__15, "Error: 8/failAsync:15/C:58/B:50/A:42");
 return T(__cb(_, function ___(__0, __16) { strictEqual(__16, "Error: 8/fail:20/failSync:21/C:58/B:50/A:42");
 return T(__cb(_, function ___(__0, __17) { strictEqual(__17, "Error: 9/failAsync:15/D:63/B:53/A:42");
 return T(__cb(_, function ___(__0, __18) { strictEqual(__18, "Error: 9/fail:20/failSync:21/D:63/B:53/A:42");
 return T(__cb(_, function ___(__0, __19) { strictEqual(__19, "END");
 return T(__cb(_, function ___(__0, __20) { strictEqual(__20, "END");
 start(); _(); }), 10, failSync); }), 10, failAsync); }), 9, failSync); }), 9, failAsync); }), 8, failSync); }), 8, failAsync); }), 7, failSync); }), 7, failAsync); }), 6, failSync); }), 6, failAsync); }), 5, failSync); }), 5, failAsync); }), 4, failSync); }), 4, failAsync); }), 3, failSync); }), 3, failAsync); }), 2, failSync); }), 2, failAsync); }), 1, failSync); }), 1, failAsync);});