...
function T(_, code, failFn) { if (!_) { return __future(T, arguments, 0); }; var __frame = new __Frame(66, "T"); try { var s, ff, end;
 fail = failFn;
 s = "{"; return (function ___(__then) { (function ___(_) { __tryCatch(_, function __$T() {

 return A(__cb(_, __frame, 4, 9, _), code); }); })(function ___(ex, __result) { __tryCatch(_, function __$T() { if (ex) {
...
asyncTest("stacks", 20, function __1(_) { if (!_) { return __future(__1, arguments, 0); }; var __frame = new __Frame(88, "__1"); try {
 ...
 start(); _(); }), 10, failSync); }), 10, failAsync); }), 9, failSync); }), 9, failAsync); }), 8, failSync); }), 8, failAsync); }), 7, failSync); }), 7, failAsync); }), 6, failSync); }), 6, failAsync); }), 5, failSync); }), 5, failAsync); }), 4, failSync); }), 4, failAsync); }), 3, failSync); }), 3, failAsync); }), 2, failSync); }), 2, failAsync); }), 1, failSync); }), 1, failAsync); } catch (e) { __frame.err(_, e); };});