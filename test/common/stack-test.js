...
function failAsync(_, code) { if (!_) { return __future(failAsync, arguments, 0); }; var __frame = new __Frame(14, "failAsync"); try {
 return _(new Error(code)); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };};


function failSync(_, code) { if (!_) { return __future(failSync, arguments, 0); }; var __frame = new __Frame(18, "failSync"); try {
 (function fail(dummy) {
 throw new Error(code);
 })(0); _(); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };};


var fail;

function A(_, code) { if (!_) { return __future(A, arguments, 0); }; var __frame = new __Frame(26, "A"); try { var i; return (function __$A(__then) {
 if ((code == 1)) {
 ...
 return _(null, "END"); })); }), code); })); }); }); }); })); }); }); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };};


function B(_, code) { if (!_) { return __future(B, arguments, 0); }; var __frame = new __Frame(47, "B"); try { return (function __$B(__then) {
 if ((code == 7)) {
 ...
 return D(__cb(_, __frame, 6, 1, _), code); }), code); })); }), code); }); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };};


function C(_, code) { if (!_) { return __future(C, arguments, 0); }; var __frame = new __Frame(56, "C"); try { return (function __$C(__then) {
 if ((code == 8)) {
 return fail(__cb(_, __frame, 2, 2, __then), code); } else { __then(); } ; })(_); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };};


function D(_, code) { if (!_) { return __future(D, arguments, 0); }; var __frame = new __Frame(61, "D"); try { return (function __$D(__then) {
 if ((code == 9)) {
 return fail(__cb(_, __frame, 2, 2, __then), code); } else { __then(); } ; })(_); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };};


function E(_, code) { if (!_) { return __future(E, arguments, 0); }; var __frame = new __Frame(66, "E"); try { return (function ___(__then) { (function ___(_) { __tryCatch(_, function __$E() {
 ...
 return _(null, ("OK " + code)); } ; })(__then); } ; })(__then); } else { _(null, __result); } ; }); }); })(function ___() { __tryCatch(_, _); }); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };};


function T(_, fn, code, failFn) { if (!_) { return __future(T, arguments, 0); }; var __frame = new __Frame(79, "T"); try { var s, ff, end;
 fail = failFn;
 ...
 return _(null, ((end < 0) ? (s + "-- end frame missing") : s.substring(0, end))); } else { _(null, __result); } ; }); }); })(function ___() { __tryCatch(_, _); }); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };};



module("flows");

asyncTest("stacks", 20, function __1(_) { if (!_) { return __future(__1, arguments, 0); }; var __frame = new __Frame(101, "__1"); try {
 ...
 start(); _(); }), A, 10, failSync); }), A, 10, failAsync); }), A, 9, failSync); }), A, 9, failAsync); }), A, 8, failSync); }), A, 8, failAsync); }), A, 7, failSync); }), A, 7, failAsync); }), A, 6, failSync); }), A, 6, failAsync); }), A, 5, failSync); }), A, 5, failAsync); }), A, 4, failSync); }), A, 4, failAsync); }), A, 3, failSync); }), A, 3, failAsync); }), A, 2, failSync); }), A, 2, failAsync); }), A, 1, failSync); }), A, 1, failAsync); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };});


asyncTest("catch", 20, function __2(_) { if (!_) { return __future(__2, arguments, 0); }; var __frame = new __Frame(125, "__2"); try {
 ...
 start(); _(); }), E, 10, failSync); }), E, 10, failAsync); }), E, 9, failSync); }), E, 9, failAsync); }), E, 8, failSync); }), E, 8, failAsync); }), E, 7, failSync); }), E, 7, failAsync); }), E, 6, failSync); }), E, 6, failAsync); }), E, 5, failSync); }), E, 5, failAsync); }), E, 4, failSync); }), E, 4, failAsync); }), E, 3, failSync); }), E, 3, failAsync); }), E, 2, failSync); }), E, 2, failAsync); }), E, 1, failSync); }), E, 1, failAsync); } catch (e) { __frame.err(_, e); } finally { __global.__frame = __frame.prev; };});