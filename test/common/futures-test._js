...
asyncTest("timeout returns before result", 3, function(_) {
	...
	try {
		equal(f(_, { timeout: 10, return: 'b' }), 'c');
	} catch (ex) {
...
asyncTest("probe with return", 2, function(_) {
	...
	equal(f(_, { timeout: 10, return: 'b', probe: true }), 'a', "probe after result");
	start();
});
asyncTest("probe with throw", 2, function(_) {
	...
	equal(f(_, { timeout: 10, throw: 'b', probe: true }), 'a', "probe after result");
	start();
});
...
asyncTest("cancel between first and second timeouts", 4, function(_) {
	...
	var f = delay2(5, 5, 'a', step);
	equals(step.i, 1, "step ok");
	equals(f(_, { timeout: 8, return: 'b' }), 'b', "timed out ok");
...
asyncTest("join returns before timeout", 3, function(_) {
	var step1 = {}, step2= {};
	var f = join(5, 10, 'a', step1, 10, 10, 'b', step2);
	equals(f(_, { timeout: 25, return: 'c' }), 'ab', "no timeout ok");
...
asyncTest("join with short timeout", 5, function(_) {
	var step1 = {}, step2= {};
	var f = join(5, 10, 'a', step1, 10, 10, 'b', step2);
	equals(f(_, { timeout: 8, return: 'c' }), 'c', "result ok");
...
asyncTest("join with intermediate timeout", 5, function(_) {
	var step1 = {}, step2= {};
	var f = join(5, 10, 'a', step1, 10, 10, 'b', step2);
	equals(f(_, { timeout: 18, return: 'c' }), 'c', "result ok");
	equals(step1.i, 3, "step1 ok");
