...
function parseOptions(argv) {
	...
	function reCheck(re) {
		return function(s) { if (!re.test(s)) throw new Error("option value " + s + " does not match " + re); };
	}
	...
	var prog = program.version(require('../package').version, '-v, --version')
		...
		.option('-s, --source-maps <true|false|inline|both>', "(see babel)", reCheck(/^(true|false|inline|both)$/))
		.option('--flow <value>', "generate .js.flow files for static type checking", reCheck(/^(true|false|only)$/))
...
function runScript(options) {
	...
	if (!args) throw new Error('internal error');
	var filename = args[0];
	...
	process.argv = [process.argv[1], filename].concat(args.slice(1));
