...
function _compileFile(_, path, options) {
	...
	var transformed = transformSync.transform(source, options);
	var dstName = outputFile(_, path, options);
	if (options.flow !== 'only') {
		var mapFile = "";
		if (options.sourceMaps && options.sourceMaps !== 'false') {
			mapFile = options.sourceMapTarget || path.replace(/\.\w+$/, '.map');
			var mapUrl;
	...
	if (options.flow && options.flow !== 'false') {
		var flowTransformed = require('babel-core').transform(source, {
			plugins: [require('babel-plugin-flow-comments'), require('babel-plugin-streamline-flow')],
		});
		var flowName = dstName.replace(/\.js$/, '.js.flow');
		if (!options.quiet) util.log("creating: " + flowName);
		fs.writeFile(flowName, flowTransformed.code, 'utf8', _);
	}
};

exports.compile = function(_ : _<void>, paths : string[], options : StreamlineOptions) : void {
