/*** Generated by streamline 0.8.1 (generators-fast) - DO NOT EDIT ***/var galaxy = require("streamline/lib/generators-fast/runtime"); galaxy.unstar((function*(_) { /**
...
(function(exports) {
	...
	Object.defineProperty(Array.prototype, 'forEach_', {
		...
		value: function*(_, options, fn, thisObj) {
			...
			if (par === 1 || len <= 1) {
				for (var i = 0; i < len; i++) {
					if (has.call(this, i)) (yield fn.call(thisObj, _, this[i], i));
				}
			} else {
	...
	Object.defineProperty(Array.prototype, 'map_', {
		...
		value: function*(_, options, fn, thisObj) {
			...
			if (par === 1 || len <= 1) {
				...
				for (var i = 0; i < len; i++) {
					if (has.call(this, i)) result[i] = (yield fn.call(thisObj, _, this[i], i));
				}
			} else {
				var fun = funnel(par);
				result = this.map(function(elt, i) {
					return galaxy.spin(fun(null, function*(_) {
						return (yield fn.call(thisObj, _, elt, i));
					}), 0);
				});
				for (var i = 0; i < len; i++) {
		...
		value: function*(_, options, fn, thisObj) {
			...
			if (par === 1 || len <= 1) {
				for (var i = 0; i < len; i++) {
					if (has.call(this, i)) {
						var elt = this[i];
						if ((yield fn.call(thisObj, _, elt))) result.push(elt);
					}
				}
			} else {
				(yield this.map_(_, par, function*(_, elt) {
					if ((yield fn.call(thisObj, _, elt))) result.push(elt);
				}, thisObj));
			}
			return result;
		...
		value: function*(_, options, fn, thisObj) {
			...
			if (par === 1 || len <= 1) {
				for (var i = 0; i < len; i++) {
					...
					if (has.call(this, i) && !(yield fn.call(thisObj, _, this[i]))) return false;
				}
			} else {
				var fun = funnel(par);
				var futures = this.map(function(elt) {
					return galaxy.spin(fun(null, function*(_) {
						return (yield fn.call(thisObj, _, elt));
					}), 0);
				});
				for (var i = 0; i < len; i++) {
	...
	Object.defineProperty(Array.prototype, 'some_', {
		...
		value: function*(_, options, fn, thisObj) {
			...
			if (par === 1 || len <= 1) {
				for (var i = 0; i < len; i++) {
					if (has.call(this, i) && (yield fn.call(thisObj, _, this[i]))) return true;
				}
			} else {
				var fun = funnel(par);
				var futures = this.map(function(elt) {
