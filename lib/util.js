...
function cacheRoot(options) {
	...
	return fsp.join(process.env.HOME || (process.env.HOMEDRIVE + process.env.HOMEPATH), ".streamline");
}
...
exports.cacheDir = function(options) {
	var subdir = 'v' + pluginVersion + '-' + options.runtime + '-' + fastHash(JSON.stringify(util.extend({}, options, {
		ext: undefined // clear ext to have a single subdir for .js and ._js
	})));
