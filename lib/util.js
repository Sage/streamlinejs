...
exports.babelOptions = function(options, filename) {
	...
	function resolvePlugin(plugin) {
		if (typeof plugin === 'string') {
			...
			if (/^(babel-plugin-)?streamine$/.test(plugin)) {
			...
			if (plugin[1] && plugin[1].asyncGenerator === false && options.runtime === 'generators') {
	...
	function resolvePreset(preset) {
		if (typeof preset === 'string') {
			return tryRequire('babel-preset', preset);
		} else if (preset && preset.plugins) {
	...
	babelOpts.presets = babelOpts.plugins.map(resolvePreset);
