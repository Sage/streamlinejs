...
function _compile(_, input, output, options) {
	...
	if (stat.isDirectory()) {
		...
		var banner = transform.banner(stat.mtime);
		if (_exists(output, _)){
			if (!options.force && fs.readFile(output, "utf8", _).substring(0, banner.length) === banner)
				return;
		}
		if (options.verbose)
			console.log("compiling " + input);
		var transformed = transform.transform(source);
