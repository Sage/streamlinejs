...
(function(exports) {
	...
	exports.runtime = function(filename, oldStyleFutures) {
		...
		function __func(_, __this, __arguments, fn, index, frame, body) {
			...
			__g.depth++;
			__g.emitter && __g.emitter.emit("resumeSlice", _); // <- This allows the event handler to detect if the callback starts a new asynchronous path.
			try {
				frame.active = true;
				body();
			} catch (e) {
				...
				frame.active = false;
				// We emit this before resetting the frame so that yieldSlice has access to the current frame.
				__g.emitter && __g.emitter.emit("yieldSlice");
	...
	function __cb(_, frame, offset, col, fn, trampo, returnArray) {
		...
		var ret = function ___(err, result) {
			...
			__g.depth === 0 && __g.emitter && __g.emitter.emit("resume");
			__g.depth++;
			__g.emitter && __g.emitter.emit("resumeSlice", undefined);
			try {
