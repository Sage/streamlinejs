		...
		var ret = function ___(err, result) {
			...
			try {
				if (trampo && frame.active && __g.trampoline) {
					...
					___.dispatched = true;
					if (err) {
						__g.emitter && __g.emitter.emit('enter', _) && (emitted = true);
						__setEF(err, frame);
						return _(err);
					}
					frame.active = true;
					__g.emitter && __g.emitter.emit("enter", fn) && (emitted = true);
					return fn(null, result);
				}
			} catch (ex) {
				if (___.dispatched && _.name !== '___' && _.name !== '__trap') throw ex;
				!emitted && __g.emitter && __g.emitter.emit('enter', _) && (emitted = true);
				__setEF(ex, frame);
