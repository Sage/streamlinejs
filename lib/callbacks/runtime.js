...
(function(exports) {
	...
	function __cb(_, frame, offset, col, fn, trampo) {
		...
		frame.col = col;
		var ctx = __g.context;
		var future = __g.future;
		return function ___(err, result) {
			if (future && future.cancelled) return;
	...
	function __future(fn, args, i) {
		...
		var prevFuture = __g.future;
		__g.future = future;
		try {
			fn.apply(this, args);
		} finally {
			__g.future = prevFuture;
		}

		function future(cb, ms, def) {
			if (!cb) return future;
			if (future.cancelled) return cb(new Error("future cancelled"));
			if (done) return cb(err, result);
			if (ms === 0) {
				if (arguments.length === 3) return cb(null, def);
				else return future.cancelled = true, cb(new Error("timeout"));
			}
			var ncb = cb;
			if (ms != null) {
