/*** Generated by streamline fibers 0.1.0 - DO NOT EDIT ***/var fstreamline__ = require("streamline/lib/fibers/runtime"); (fstreamline__.create(function(_) {var _mkdir_ = fstreamline__.create(_mkdir), mtime_ = fstreamline__.create(mtime), mkdirs_ = fstreamline__.create(mkdirs), cachedTransform_ = fstreamline__.create(cachedTransform); //streamline.options = { "lines" : "preserve" }
"use strict";
...
function _mkdir(dir, mode, _) {
	...
	if (!fstreamline__.invoke(null, _exists, [p, _], 1)) fstreamline__.invoke(null, _mkdir_, [p, mode, _], 2);
...
exports.loadFile = fstreamline__.create(function(_, path, options) {
	...
	if (mtimejs_) {
		...
		if (matches = streamlineRE.exec(content)) {
			...
			return fstreamline__.invoke(null, cachedTransform_, [_, content, path, transform, banner, options], 0);
		} else {
			return content;
		}
	}
...
function mkdirs(_, path) {
	var p = "",
		i = 0;
	path.split('/').slice(0, -1).forEach(function(seg) { var __tmp;
		 __tmp = (__tmp = (i++ ? '/' : '') + seg, p += __tmp);
...
function cachedTransform(_, content, path, transform, banner, options) {
	...
	fstreamline__.invoke(null, mkdirs_, [_, f], 0);
	var transformed;
	if (fstreamline__.invoke(null, mtime_, [_, f], 0) > fstreamline__.invoke(null, mtime_, [_, path], 0)) {
		transformed = fstreamline__.invoke(fs, "readFile", [f, "utf8", _], 2);
	...
	if (path.indexOf('/tmp--') < 0) fstreamline__.invoke(fs, "writeFile", [f, transformed, "utf8", _], 3);
	return transformed;
}

function mkdirsSync(path) {
	var p = "";
	path.split('/').slice(1, -1).forEach(function(seg) { var __tmp;
		 __tmp = (__tmp = '/' + seg, p += __tmp);
