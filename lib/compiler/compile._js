...
function transformFile(path, options, _) {
	...
	if (/\._coffee$/.test(path)) {
		...
		return babel.transform(util.removeShebang(fs.readFile(path, 'utf8', _)), babelOptions);	
	}
}
...
exports.compileFile = function(_, path, options) {
	...
	if (options.sourceMap && transformed.map) {
...
exports.loadFile = function(_, path, options) {
	...
	return banner + cache.getCode(_, path, options, function(_) {
		return transformFile(path, options, _);
	});
};
...
exports.transform = function(_, path, options) {
	...
	return cache(_, path, options, function(_) {
		return transformFile(path, options, _);
	});
};
