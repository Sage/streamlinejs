...
exports.transformModule = function(content, path, options) {
	...
	if (ext !== '.js' && ext !== '.coffee') {
		...
		return banner + require('babel').transform(content, babelOptions).code;
	} else {
		return content;
	}
};
...
exports.cachedTransformSync = function(content, path, transform, options, testOnly) {
	...
	var babelOptions = util.babelOptions(options,  "transformFile");
	return cache.sync(path, options, function() {
		return banner + require('babel').transformFileSync(path, babelOptions).code;
	});
};
