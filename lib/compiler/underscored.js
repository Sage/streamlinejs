...
function run(options) {
	...
	var streamliners = {
		_js: function(module, filename, code) {
			...
			_compile.call(module, streamlined, filename)
		},
	...
	// TypeScript hook
	// We cannot easily control file extensions. So we have to look for a special marker in .js files.
	Module.prototype._compile = function(code, filename) {
		var doIt;
		if (filename.substring(filename.length - 3) === '.js') {
			code = code.replace(/^~~~streamline\(\);/m, function() {
				doIt = true;
				return '';
			});
		}
		if (doIt) code = code.replace(/\b(array_|future_)\(/g, '(');
		return doIt ? streamliners._js(this, filename, code) : _compile.call(this, code, filename);
	}
	...
	var isJs = filename.substring(filename.length - 3) === '.js';
	require.extensions[isJs ? '.js' : coffeePresent ? '._coffee' : '._js'](mainModule, filename);
}
