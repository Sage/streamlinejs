...
function createTemplate(fn, idx, file, line) {
	...
	function F() {
		...
		var that = this,
			args = arguments;
		var emitter = globals.emitter, frame;
		if (emitter) frame = globals.frame;
		...
		// Tricky: Fiber.run may run code from other fibers.
		if (emitter) globals.frame = frame;
	};

	// Memoize the original function for fast passing later
	F.fstreamlineFunction = fn;
	F.fstreamlineFile = file;
	F.fstreamlineLine = line;
	return F;
}
...
function pushFrame(fn) {
	var frame = globals.frame = {
		prev: globals.frame,
		name: fn.fstreamlineFunction.name,
		file: fn.fstreamlineFile,
		line: fn.fstreamlineLine,
	}
...
function popFrame(frame) {
	if (globals.frame !== frame) throw new Error("FRAME MISMATCH1: \n\t" + JSON.stringify(frame) + '\n\t' + JSON.stringify(globals.frame));
	globals.emitter.emit('exit');
