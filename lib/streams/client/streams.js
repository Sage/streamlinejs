/*** Generated by streamline 0.12.1 (callbacks) - DO NOT EDIT ***/ "use strict"; var __rt=require('../callbacks/runtime').runtime(__filename, false),__func=__rt.__func,__cb=__rt.__cb; 
...
function DataBuffer(options) { 
    ...
    this.read = function read__1(_, len) { var chunks, total, chunk; var __frame = { name: 'read__1', line: 17 }; return __func(_, this, arguments, read__1, 0, __frame, function __$read__1() { 
            ...
            if (len === 0) { return _(null, ''); } chunks = []; 
            total = 0; 
            
            while (total < len) { chunk = _chunks.splice(0, 1)[0]; 
                
                if (!chunk) { return _(null, chunks.length === 0 ? null : chunks.join('')); } 
                if (total + chunk.length <= len) { 
                    chunks.push(chunk); 
                    total += chunk.length; } else { 
    ...
    this.readAll = function readAll__2(_) { var __this = this; var __frame = { name: 'readAll__2', line: 39 }; return __func(_, this, arguments, readAll__2, 0, __frame, function __$readAll__2() { 
            return __this.read(__cb(_, __frame, 1, 0, _, true, false), -1); }); }; 
...
function HttpClientRequest(options) { 
    ...
    this.response = function (callback) { 
        ...
        $.ajax({ url: options.url, headers: options.headers, type: options.method, data: this.contents(), dataType: 'text', beforeSend: function (xhr) { 
                ...
                _xhr = xhr; }, success: function (data, statusText, xhr) { 
                
                
                callback(null, new HttpClientResponse(data, xhr)); }, error: function (xhr, statusText, message) { 
                
                
                if (statusText === 'error') callback(new HttpError(xhr.status, statusText + ': ' + message)); else callback(new HttpError(400, statusText + ': ' + message)); } }); }; 
