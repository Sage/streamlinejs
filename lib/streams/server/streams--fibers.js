...
function Emitter(emitter){
	...
	this.close = fstreamline__.create(function(_){
		return fstreamline__.invoke(null, (function(callback){
			if (!emitter) 
...
function ReadableStream(emitter, options) {
	...
	function trackData(err, chunk) { var __tmp;
		if (!emitter)
			return;
		if (err)
			_error = err;
			...
			_chunks.push(chunk);
			 __tmp = (__tmp = chunk.length, _current += __tmp);
	...
	function readChunk(callback) { var __tmp;
		if (_chunks.length > 0) {
			...
			_onData = function(err, chunk){ var __tmp;
				if (err) 
					_error = err;
				else if (!chunk) 
					_done = true;
	...
	this.read = fstreamline__.create(function(_, len) { var __tmp;
		if (!this.emitter)
			return null;
		if (len == null)
			return fstreamline__.invoke(null, readChunk, [_], 0);
		if (len < 0)
			len = Infinity;
		if (len == 0)
		...
		var chunks = [], total = 0;
		while (total < len) {
			var chunk = fstreamline__.invoke(null, readChunk, [_], 0);
			if (!chunk)
				return chunks.length == 0 ? null : concat(chunks, total);
			if (total + chunk.length <= len) {
				chunks.push(chunk);
				 __tmp = (__tmp = chunk.length, total += __tmp);
			} else {
	...
	///   returns `this` for chaining.
	this.unread = function(chunk) { var __tmp;
		if (chunk) {
...
function WritableStream(emitter, options) {
	...
	this.write = fstreamline__.create(function(_, data, enc) {
		...
		if (!emitter.write(data, enc))
			fstreamline__.invoke(null, _drain, [_], 0);
		return this;
	}, 0)
	/// * `stream.end()`  
...
function HttpServer(requestListener, options) {
	...
	///   (same as `http.Server`)
	this.listen = fstreamline__.create(function(_, args){
...
function HttpClientRequest(options) {
	...
	this.response = fstreamline__.create(function(_) {
		return fstreamline__.invoke(null, (function(callback){
			if (_done) 
...
function NetClient(options, args) {
	...
	this.connect = fstreamline__.create(function(_){
		return fstreamline__.invoke(null, (function(callback){
			if (_done) 
