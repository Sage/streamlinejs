...
function WritableStream(emitter, options) {
 ...
 checkOpen(__this); return (function __$write__1(__then) {
...
function HttpServer(requestListener, options) {
 ...
 return (function(callback) { args.push(function() { _reply(callback, null, self); }); server.listen.apply(server, args); })(__cb(_, __frame, 7, 9, _)); }); };};
...
function HttpClientRequest(options) {
 ...
 return (function(callback) { if (_done) { return _reply(callback, _error, _response); } else { _onResponse = function(err, resp) { _reply(callback, err, resp); }; }; })(__cb(_, __frame, 8, 9, _)); }); };};
...
function NetClient(options, args) {
 ...
 return (function(callback) { if (_done) { return _reply(callback, _error, new NetStream(_connection, options)); } else { _onConnect = function(err) { _reply(callback, err, new NetStream(_connection, options)); }; }; })(__cb(_, __frame, 8, 9, _)); }); };};