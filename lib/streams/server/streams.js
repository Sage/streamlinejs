...
function checkOpen(stream) {
	if (!stream.emitter) throw new Error("invalid operation on closed stream")
}
...
function Emitter(emitter) {
	...
	this.close = function(_) {
		return (function(callback) {
			if (!emitter) return _reply(callback);
			var close = emitter.close || emitter.destroySoon;
			if (typeof close !== "function") return _reply(callback);
...
function ReadableStream(emitter, options) {
	...
	this.read = function(_, len) {
		if (!this.emitter && !_chunks.length) return null;
...
function HttpClientRequest(options) {
	...
	this.response = function(_) {
		return (function(callback) {
			if (_done) return _reply(callback, _error, _response);
...
function NetClient(options, args) {
	...
	this.connect = function(_) {
		return (function(callback) {
			if (_done) return _reply(callback, _error, new NetStream(_connection, options));
