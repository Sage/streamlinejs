...
function wrapProperties(constr, writable, props) {
  props.forEach(function __1(name) {
    constr.prototype.__defineGetter__(name, function __1() {
      return this.emitter[name];
    });
  });
  (writable && props.forEach(function __2(name) {
    constr.prototype.__defineSetter__(name, function __1(val) {
...
function wrapMethods(constr, methods) {
  methods.forEach(function __1(name) {
    constr.prototype[name] = function __1() {
...
function Emitter(emitter) {
  ...
  this.close = function __2(callback) {
    if (!callback) {
  ...
  this.__defineGetter__("emitter", function __3() {
    return emitter;
  });
  this.unwrap = function __4() {
    var result = emitter;
    (emitter && emitter.events.forEach(function __1(event) {
...
function ReadableStream(emitter, options) {
  ...
  var _done = false;
  var _encoding;
  emitter.on("error", function __1(err) {
    _onData(err);
  });
  emitter.on("data", function __2(chunk) {
    _onData(null, chunk);
  });
  emitter.on("end", function __3() {
    _onData(null, null);
  });
    ...
    ;
      ...
      return function(__break) {
        var __loop = __nt(_, function() {
          var __then = __loop;
          if ((total < len)) {
            return readChunk(__cb(_, function(__0, chunk) {
              if (!chunk) {
  ...
  this.unread = function __7(chunk) {
    if (chunk) {
...
function WritableStream(emitter, options) {
  ...
  this.write = function __2(_, data, enc) {
    if (!_) {
  ...
  ;
    ...
    return function(__then) {
      if (!emitter.write(data, enc)) {
        return _drain(__cb(_, __then.bind(this)));
      }
    ;
      return __then();
    }.call(this, function() {
...
function HttpServer(requestListener, options) {
  ...
  var server = options.module.createServer(function __1(request, response) {
    return requestListener(new HttpServerRequest(request, options), new HttpServerResponse(response, options), function __1(err) {
      if (err) {
  ...
  this.listen = function __2(callback, args) {
    if (!callback) {
      return __future.call(this, this.listen, arguments, 0)
    };
    args = Array.prototype.slice.call(arguments, 1);
    args.push(function __1() {
...
HttpClientResponse.prototype.checkStatus = function __1(statuses) {
...
exports.httpRequest = function __2(options) {
...
var net;
exports.tcpClient = function __3(port, host, options) {
  host = (host || "localhost");
  options = (options || {
  });
  return new NetClient(options, port, host);
};
exports.socketClient = function __4(path, options) {
  options = (options || {
  });
