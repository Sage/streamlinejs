			...
			if (path) {
				...
				each(_, missing, function(_, dep) {
					try {
						...
						var data = require("streamline/lib/compiler/compile").loadFile(_, dep + ".js");
						if (config.getResources) data += ";module.__resources$$=" + config.getResources(_, dep + ".js", request.headers['accept-language']) + ";";
				...
				each(_, missing, function(_, dep) {
					try {
						var modIndex = dep.indexOf("/node_modules/");
						var location = modIndex >= 0 ? dep.substring(modIndex + 14) : dep.substring(root.length);
						var data = config.getResources(_, dep + ".js", locale);
