/*** Generated by streamline fibers 0.1.0 - DO NOT EDIT ***/var fstreamline__ = require("streamline/lib/fibers/runtime"); (fstreamline__.create(function(_) { "use strict";
...
exports.dispatcher = function(config) {
	...
	return fstreamline__.create(function(_, request, response) {
		try {
			...
			var stats = fstreamline__.invoke(fs, "stat", [path + ".js", _], 1);
			if (!stats.isFile())
				return _replyError(response, 404, "file not found " + path);

			var missing = fstreamline__.invoke(depend, "missingDependencies", [_, path, known], 0);
			...
			if (accept.indexOf('text/html') == 0) {
				...
				fstreamline__.invoke(response, "write", [_, "<html>"
			...
			var i = 0;
			fstreamline__.invoke(null, each, [_, missing, fstreamline__.create(function(_, dep) {
				try {
					var modIndex = dep.indexOf("/node_modules/");
					var location = modIndex >= 0 ? dep.substring(modIndex + 14) : dep.substring(root.length);
					var data = fstreamline__.invoke(fs, "readFile", [dep + ".js", "utf8", _], 2);
					fstreamline__.invoke(response, "write", [_, "\n--" + boundary + "\n"
					+ "Content-ID: FILE " + ++i + "\n"
